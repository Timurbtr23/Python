    ...
    def mycommand(self):
        ...
        try:
            ...
            if ((statement.find("читать") != -1) or (statement.find("четать") != -1) or (statement.find("читати") != -1)
                    or (statement.find("читает") != -1) or (statement.find("чи тать") != -1) or (
            ...
            if ((statement.find("пагода") != -1) or (statement.find("погода") != -1) or (
                    statement.find("какая погода") != -1) or (statement.find("какая пагода") != -1) or (
                    statement.find("погода сегодня") != -1)):
                statement = self.cleanphrase(statement, ['погода'])
                self.openurl('https://google.com//search?q=погода' + statement, "Я нашла следующие результаты")
                mflag = 1
            ...
            if (((statement.find("youtube") != -1) or (statement.find("ютуб") != -1) or (
                    statement.find("you tube") != -1)) and (statement.find("смотреть") != -1)):
                statement = self.cleanphrase(statement,
                                             ['хочу', 'на ютубе', 'на ютуб', 'на youtube', 'на you tube', 'на youtub',
            ...
            if ((statement.find("слушать") != -1) and (statement.find("песн") != -1)):
                ...
                self.openurl('https://music.yandex.ru/search?text=' + statement, "Нажмите плэй")
                mflag = 1

            if ((statement.find("кто ты") != -1) or (statement.find("о себе") != -1) or (statement.find("расскажи о себе") != -1)
